...
- scrape.yaml: 경남신문(홈페이지 스크래핑, 63건)·웅상신문(모바일 목록, 2026-08-15 제호 변경: 양산웅상신문) 추가
- scrape.py: 웅상신문 블록 단위 파싱(경계 침범 방지) + news_modify 날짜 파싱 + 48시간 필터 — 목록이 오래된 순이라 2022~2023년 기사가 '오늘 뉴스'로 실리는 문제 방지
- PRD/CLAUDE.md: 섹션 3·4·5 상태 표기 정정(이미 활성화됨), 경남신문 수집 가능 정정(JS 로딩 아님), 섹션 개수 9→10(CLAUDE.md 60행만)
- 테스트: 양산 전용 섹션 5개 + scrape 신규 3개 (총 62개 통과)

## 2026-08-19 — 지자체·공공기관 보도자료 수집 연동 (도청·양산·창원·교육청)

- feeds.yaml: 경남도청 공식 보도자료 RSS 피드 추가 (공식 RSS 2.0 직접 수집)
- scrape.yaml: 양산시청, 창원시청, 경남교육청 3개 지자체·교육청 보도자료 스크래핑 소스 등록
- scrape.py:
  - 양산시청: `newsEpctNo` 파라미터 및 읍·면·동 단순 미담/나눔 필터링 파서 추가 (AI 토큰 비용 방어)
  - 창원시청: `10432.web` 목록에서 제목·요약문·담당부서·날짜 추출 및 `&amp;` 언이스케이프 URL 조립 파서 추가
  - 경남교육청: 교육홍보관 보도자료 목록에서 제목·요약문·날짜 추출 파서 추가
  - 목록에서 이미 요약문(`summary`)이 추출된 기사는 Jina Reader 외부 본문 조회를 생략하도록 최적화
- 테스트: `tests/test_scrape.py`에 지자체 3곳 단위 테스트 추가 (총 65개 테스트 전체 통과)
- 실측 검증: 경남도청(10건), 양산시청(6건), 창원시청(10건), 경남교육청(6건) 실시간 정상 수집 확인