...
매일 아침·저녁, RSS·스크래핑으로 뉴스를 수집하여 AI로 필터링·요약하고 텔레그램으로 발송한다.

- **전체 기획**: `PRD.md` 또는 `docs/PRD.md` 참조 (반드시 작업 전 확인)
- **개발자**: 레고 (비개발자, Claude Code로 개발 주도)
- **현재 단계와 로드맵**: `PRD.md` 참조 (Phase 0~4 완료)
...
- **AI**: 2단계 구조 (싼 모델로 거르고, 좋은 모델로 요약)
  - **GPT-5.6 Luna** (OpenAI) — 1차 분류 (카테고리·중요도·스코프)
  - **Sonnet 4.6** (Anthropic) — 2차 요약·코멘트 생성 (발송 대상 기사만)
- **저장소**: SQLite (`state/seen.db` — 원격에 매 실행마다 커밋됨)
- **발송**: python-telegram-bot (텔레그램 전용)
- **스케줄링**: cron-job.org가 GitHub Actions를 깨움 (저장소 안에 시간표 없음)
- **RSS**: feedparser
...
- 10개 섹션 중 특정 섹션에만 기사가 몰리지 않도록 RSS 피드 분산 구성

## 프로젝트 구조

```
clipboard055/
├── .github/workflows/     # GitHub Actions (cron-job.org가 깨움)
├── src/                   # 소스 코드
│   ├── main.py           # 파이프라인 오케스트레이션
│   ├── collect.py        # RSS 수집
│   ├── scrape.py         # 웹 스크래핑 (경남 매체)
│   ├── dedupe.py         # 중복 제거
│   ├── classify.py       # Luna 분류 (9 카테고리)
│   ├── summarize.py      # Sonnet 요약
│   ├── section_builder.py # 섹션 구성·묶기·분량 제한
│   ├── keyword_boost.py  # 키워드 가산·사전 필터
...
│   ├── scrape.yaml       # 스크래핑 대상
│   └── keywords.yaml     # 필터링·가산·분량 설정
├── state/                # SQLite (원격에 매 실행마다 커밋)
...
9개 카테고리·섹션 매핑 목록은 `PRD.md`의 섹션 구성 표와 `src/section_builder.py`의 `SECTION_DEFS` 참조.
...
### 단계별 개발
- **PRD의 Phase 순서를 따른다**. 현재 Phase와 활성 섹션은 `PRD.md` 참조
- Phase를 건너뛰지 않는다
...
`.env.example` 참조. 실제 값은 `.env`에 저장 (gitignore):

- `ANTHROPIC_API_KEY` — Sonnet 요약용
- `OPENAI_API_KEY` — Luna 분류용
- `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHANNEL_ID`, `TELEGRAM_ADMIN_USER_ID` — 텔레그램 발송·오류 알림
- `LOG_LEVEL`, `TIMEZONE` — 로그·시간대
...
2. `PRD.md` — 무엇을 만드는가, 현재 상태, 로드맵
3. `docs/todolist.md` — 다음에 할 일 (항목마다 왜/어느 파일/어떤 방향)
4. `docs/worklog.md` — 작업 기록 (쌓기만 함, 위를 고치지 않음)
